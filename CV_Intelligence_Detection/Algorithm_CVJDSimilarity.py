...
def get_collection_jdtitle_match(jdtitle, cvlists):
    """
    """
    jdtitle = set(jdtitle)
    match = []
    for cvlist in cvlists:
        cvlist = set(cvlist)
        crosswords = jdtitle.intersection(cvlist)
        if len(crosswords) == 0:
            match.append(0.1)
        elif float(len(crosswords))/float(len(jdtitle)) < 0.1:
            match.append(0.1)
        else:
            match.append(float(len(crosswords))/float(len(jdtitle)))
    return match

def get_collection_similaritywithfactors(cvlists, jdlist, jdtitlelist=[]):
    """
    """
    if len(jdtitlelist) == 0:
        return get_collection_similarity(cvlists, jdlist)
    else:
        term_similarity = get_collection_similarity(cvlists, jdlist)
        jd_match = get_collection_jdtitle_match(jdtitlelist, cvlists)
        return lib_np.array(term_similarity)*lib_np.array(jd_match)        